...
export function getSuggestions() {
  ...
  fetch("./data/citylist.json").then(response => response.json()).then(cities => {
    variables.cityName.addEventListener("input", event => {
      ...
      if (inputValue.length >= 3) {
        var suggestions = [...new Set(cities.filter(city => city.name.toLowerCase().startsWith(inputValue.toLowerCase())).map(city => city.name).sort())];
